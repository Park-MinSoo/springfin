...
public class ResponseBodyController {
	@RequestMapping(value = "/body/text/{id}", produces = "text/plain; charset=utf-8")//{id}를 설정해주며 동적 패스 variable을 설정하고있다. 만약 이것을 안주게 되면 Query에서 추출하게 되는 것., plain 이기 때문에 html의 태그 <h1>을 못 알아먹고 그냥 그대로 일반 텍스트처럼 출력하게 된다.
	@ResponseBody//view를 안거치고 컨트롤라가 직접 응답 body를 구성하겠다는 뜻이다. 그러기 위해선 produces로 응답 타입과 character set을 지정해 주어야 한다.
